...
wss.on('connection', (ws, req) => {
  ...
  broadcast(room, { type: 'system', text: name + ' joined the room' }, ws);
  ws.send(JSON.stringify({ type: 'system', text: 'Welcome to room ' + room }));
  ws.on('message', (data) => {
    try {
  ...
  ws.on('close', () => {
    ...
    broadcast(room, { type: 'system', text: name + ' left the room' }, null);
  });
});
